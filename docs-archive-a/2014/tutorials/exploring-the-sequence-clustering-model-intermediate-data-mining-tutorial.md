...
1.  Fare clic con il pulsante destro del mouse `Pacific Cluster` , scegliere **drill-through**, quindi **colonne struttura e modello**.  
...
1.  Fare clic con il pulsante destro del mouse sulla riga del **modello** nella colonna per il cluster `Largest Cluster` e selezionare **Mostra legenda**.  
...
1.  Scorrere verso il basso nel riquadro attributo fino a trovare la riga per l' `Region` attributo.  
...
3.  Nella `Region` riga, posizionare il puntatore del mouse sull'istogramma colorato per `Largest Cluster` .  
  
     I valori visualizzati nella descrizione comando indicano le percentuali effettive dei case di ogni area.  
  
4.  Fare clic con il pulsante destro del mouse sull'istogramma colorato nella `Region` riga per `Pacific Cluster` , selezionare **drill-through**, quindi selezionare **solo colonne modello**.  
...
7.  Nell'elenco a discesa **cluster** selezionare **popolamento (tutti)**.  
 ...
 Il secondo è costituito da un forte impatto sul clustering sugli attributi per `Region` , che rende più difficile la visualizzazione dei gruppi di sequenze. Si decide pertanto di creare un altro modello che utilizza solo le sequenze e non include le colonne relative a area o reddito.  
