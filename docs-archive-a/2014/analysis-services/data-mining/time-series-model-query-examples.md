...
###  <a name="sample-query-1-retrieving-periodicity-hints-for-the-model"></a><a name="bkmk_Query1"></a> Query di esempio 1: Recupero di hint di periodicità per il modello  
...
###  <a name="sample-query-2-retrieving-the-equation-for-an-arima-model"></a><a name="bkmk_Query2"></a> Query di esempio 2: Recupero dell'equazione per un modello ARIMA  
...
###  <a name="sample-query-3-retrieving-the-equation-for-an-artxp-model"></a><a name="bkmk_Query3"></a>Esempio di query 3: recupero dell'equazione per un modello ARTXP  
...
-   Usando una query di stima singleton per specificare i dati un intervallo alla volta. Per informazioni su come creare una query di stima singleton, vedere [interfacce di query di data mining](data-mining-query-tools.md).  
  
###  <a name="understanding-the-behavior-of-replace-and-extend-operations"></a><a name="bkmk_ReplaceExtend"></a> Informazioni sul comportamento delle operazioni di sostituzione ed estensione  
...
###  <a name="making-predictions-with-replace_model_cases"></a><a name="bkmk_REPLACE"></a>Esecuzione di stime con REPLACE_MODEL_CASES  
 ...
 Di seguito è riportato un altro esempio. si supponga di volere aggiungere nuovi dati relativi a tre mesi ed eseguire tre nuove stime. In tal caso, effettuare quanto segue:  
