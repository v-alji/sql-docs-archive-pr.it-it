...
##  <a name="example-1-parsing-row-delimited-records"></a><a name="example1"></a>Esempio 1: analisi di record delimitati da righe  
...
13. Nella pagina **script** di **Editor trasformazione script**fare clic su **Modifica script** e immettere il codice illustrato nella `ScriptMain` classe dell'esempio. Chiudere l'ambiente di sviluppo dello script e l'**Editor trasformazione Script**.  
...
13. Creare un secondo output e denominarlo ChildRecords. L'oggetto `SynchronousInputID` del nuovo output è già impostato su Nessuno. Creare 3 colonne di output:  
