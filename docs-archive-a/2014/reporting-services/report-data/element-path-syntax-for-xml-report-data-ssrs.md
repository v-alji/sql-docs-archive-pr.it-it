...
> [!NOTE]  
>  La sintassi del percorso di elemento è indipendente dallo spazio dei nomi. Per utilizzare gli spazi dei nomi in un percorso di elemento, utilizzare la sintassi di query XML che include un `ElementPath` elemento XML descritto nella [sintassi XML query per i dati del Report XML &#40;&#41;SSRS ](report-data-ssrs.md).  
...
|`ElementNode`|Nodo XML nel documento XML. I nodi sono designati da tag e sono correlati agli altri nodi in base a una relazione gerarchica. Ad esempio, \<Customers> è il nodo elemento radice. \<Customer>è un sottoelemento di \<Customers> .|  
...
|`Attribute`|Coppia nome-valore contenuta in `ElementNode`. Nel nodo elemento, ad esempio \<Customer ID="1"> , `ID` è un attributo e `@ID(Integer)` restituisce "1" come tipo integer nel campo dati corrispondente `ID` .|  
|`Value`|Valore dell'elemento. `Value` può essere utilizzato solo nell'ultimo `ElementNode` del percorso di elemento. Ad esempio, poiché \<Return> è un nodo foglia, se lo si include alla fine di un percorso di elemento, il valore di `Return {@}` è `Chair` .|  
