...
## <a name="how-to-confirm-a-report-server-initialization"></a>Come confermare l'inizializzazione di un server di report  
 Per confermare l'inizializzazione del server di report, effettuare il ping del servizio Web ReportServer digitando **http:// \<servername> /ReportServer** nella finestra di comando. Se si verifica l'errore `RSReportServerNotActivated`, l'inizializzazione non è riuscita.  
