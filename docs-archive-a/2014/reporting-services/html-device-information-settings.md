...
|`LinkTarget`|Destinazione dei collegamenti ipertestuali nel report. È possibile fare riferimento a una finestra o a un frame specificando il nome della finestra, ad esempio `LinkTarget` = *window_name*, oppure è possibile usare come destinazione una nuova finestra usando `LinkTarget` = _Blank. Altri nomi di destinazione validi includono _self, _parent e _top.|  
