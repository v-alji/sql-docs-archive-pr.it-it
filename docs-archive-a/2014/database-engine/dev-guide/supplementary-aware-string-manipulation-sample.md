...
ms.locfileid: "87716703"
---
# <a name="supplementary-aware-string-manipulation-sample"></a><span data-ttu-id="380ea-102">Esempio di modifica di stringhe in grado di riconoscere caratteri supplementari</span><span class="sxs-lookup"><span data-stu-id="380ea-102">Supplementary-Aware String Manipulation Sample</span></span>
  <span data-ttu-id="380ea-103">In questo esempio per [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] viene illustrata l'elaborazione di stringhe con supporto per i caratteri supplementari.</span><span class="sxs-lookup"><span data-stu-id="380ea-103">This sample for [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] demonstrates supplementary character aware string processing.</span></span> <span data-ttu-id="380ea-104">Nell'esempio viene illustrata l'implementazione di cinque funzioni per i valori stringa Transact-SQL che offrono le stesse funzioni di manipolazione dei valori stringa di quelle predefinite, nonché ulteriori capacità di supporto dei caratteri supplementari, che consentono di gestire sia le stringhe Unicode sia quelle contenenti caratteri supplementari.</span><span class="sxs-lookup"><span data-stu-id="380ea-104">This sample shows the implementation of five Transact-SQL string functions that provide the same string manipulation functions as the built-in functions, but with additional supplementary character-aware capability to handle both Unicode and supplementary character strings.</span></span> <span data-ttu-id="380ea-105">Le cinque funzioni sono Lens () `lefts(), rights(), subs()` e `replace_s()` sono equivalenti alle funzioni predefinite e alle funzioni di `LEN(), LEFT(), RIGHT(), SUBSTRING()` `REPLACE()` stringa.</span><span class="sxs-lookup"><span data-stu-id="380ea-105">The five functions are lens(), `lefts(), rights(), subs()` and `replace_s()` which are equivalent to the built-in functions `LEN(), LEFT(), RIGHT(), SUBSTRING()` and `REPLACE()` string functions.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="380ea-106">Prerequisiti</span><span class="sxs-lookup"><span data-stu-id="380ea-106">Prerequisites</span></span>  
 <span data-ttu-id="380ea-107">Per creare ed eseguire questo progetto, è necessario installare il software seguente:</span><span class="sxs-lookup"><span data-stu-id="380ea-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="380ea-108">o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="380ea-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="380ea-109">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express è disponibile gratuitamente nel [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sito Web [di documentazione ed esempi di](https://www.microsoft.com/sql-server/sql-server-editions-express)Express</span><span class="sxs-lookup"><span data-stu-id="380ea-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="380ea-110">Database AdventureWorks, disponibile nel [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sito Web [per sviluppatori di](https://go.microsoft.com/fwlink/?linkid=62796)</span><span class="sxs-lookup"><span data-stu-id="380ea-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="380ea-111">.NET Framework SDK 2.0 o versione successiva oppure Microsoft Visual Studio 2005 o versione successiva.</span><span class="sxs-lookup"><span data-stu-id="380ea-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="380ea-112">.NET Framework SDK è disponibile gratuitamente.</span><span class="sxs-lookup"><span data-stu-id="380ea-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="380ea-113">È necessario inoltre che siano soddisfatte le condizioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="380ea-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="380ea-114">Per l'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] usata deve essere abilitata l'integrazione con CLR.</span><span class="sxs-lookup"><span data-stu-id="380ea-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="380ea-115">Per abilitare l'integrazione con CLR, effettuare le operazioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="380ea-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="380ea-116">Abilitazione dell'integrazione con CLR</span><span class="sxs-lookup"><span data-stu-id="380ea-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="380ea-117">Eseguire i comandi [!INCLUDE[tsql](../../includes/tsql-md.md)] seguenti:</span><span class="sxs-lookup"><span data-stu-id="380ea-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="380ea-118">Per abilitare CLR, è necessario disporre dell'autorizzazione `ALTER SETTINGS` a livello di server, che viene assegnata implicitamente ai membri dei ruoli predefiniti del server `sysadmin` e `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="380ea-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="380ea-119">Il database AdventureWorks deve essere installato nell'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] in uso.</span><span class="sxs-lookup"><span data-stu-id="380ea-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="380ea-120">Se non si è un amministratore per l' [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] istanza di in uso, è necessario disporre di un amministratore per concedere l'autorizzazione **CreateAssembly** per completare l'installazione.</span><span class="sxs-lookup"><span data-stu-id="380ea-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="380ea-121">Compilazione dell'esempio</span><span class="sxs-lookup"><span data-stu-id="380ea-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="380ea-122">Creare ed eseguire l'esempio tramite le istruzioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="380ea-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="380ea-123">Aprire un prompt dei comandi di .NET Framework o Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="380ea-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="380ea-124">Se necessario, creare una directory per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="380ea-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="380ea-125">Per questo esempio verrà utilizzata la directory C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="380ea-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="380ea-126">Poiché per questo esempio è necessario un assembly firmato, creare una chiave asimmetrica digitando il comando:</span><span class="sxs-lookup"><span data-stu-id="380ea-126">Since this example requires a signed assembly, create an asymmetric key by typing the command:</span></span>  
  
     `sn -k SampleKey.snk`  
  
4.  <span data-ttu-id="380ea-127">Compilare il codice di esempio dal prompt della riga di comando eseguendo una delle istruzioni seguenti, a seconda del linguaggio scelto.</span><span class="sxs-lookup"><span data-stu-id="380ea-127">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
5.  <span data-ttu-id="380ea-128">Copiare il codice di installazione [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `Install.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="380ea-128">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
6.  <span data-ttu-id="380ea-129">Distribuire l'assembly e la stored procedure eseguendo</span><span class="sxs-lookup"><span data-stu-id="380ea-129">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql -v root = "C:\MySample\"`  
  
7.  <span data-ttu-id="380ea-130">Copiare lo script di comandi di test [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `test.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="380ea-130">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
8.  <span data-ttu-id="380ea-131">Eseguire lo script di test con il comando seguente</span><span class="sxs-lookup"><span data-stu-id="380ea-131">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
9. <span data-ttu-id="380ea-132">Copiare lo script di pulizia [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `cleanup.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="380ea-132">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
10. <span data-ttu-id="380ea-133">Eseguire lo script con il comando seguente</span><span class="sxs-lookup"><span data-stu-id="380ea-133">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="380ea-134">Codice di esempio</span><span class="sxs-lookup"><span data-stu-id="380ea-134">Sample Code</span></span>  
 <span data-ttu-id="380ea-135">Di seguito sono illustrati i listati di codice per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="380ea-135">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="380ea-136">C#</span><span class="sxs-lookup"><span data-stu-id="380ea-136">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="380ea-137">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="380ea-137">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="380ea-138">Di seguito è illustrato lo script di installazione [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) che consente la distribuzione dell'assembly e la creazione delle funzioni con supporto dei caratteri supplementari nel database.</span><span class="sxs-lookup"><span data-stu-id="380ea-138">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the supplementary-aware functions in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="380ea-139">Di seguito è illustrato lo script `test.sql`che consente di testare l'esempio eseguendo le funzioni.</span><span class="sxs-lookup"><span data-stu-id="380ea-139">This is `test.sql`, which tests the sample by executing the functions.</span></span>  
...
```  
 ...
 <span data-ttu-id="380ea-140">Il codice [!INCLUDE[tsql](../../includes/tsql-md.md)] seguente consente di rimuovere l'assembly e le funzioni dal database.</span><span class="sxs-lookup"><span data-stu-id="380ea-140">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and functions from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="380ea-141">Vedere anche</span><span class="sxs-lookup"><span data-stu-id="380ea-141">See Also</span></span>  
 [<span data-ttu-id="380ea-142">Scenari di utilizzo ed esempi per l'integrazione con CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="380ea-142">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
