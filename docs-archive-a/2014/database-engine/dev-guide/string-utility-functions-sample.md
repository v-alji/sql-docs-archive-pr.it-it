...
ms.locfileid: "87716704"
---
# <a name="string-utility-functions-sample"></a><span data-ttu-id="e750c-102">Esempio String Utility Functions</span><span class="sxs-lookup"><span data-stu-id="e750c-102">String Utility Functions Sample</span></span>
  <span data-ttu-id="e750c-103">L'esempio String Utilities include una funzione di flusso con valori di tabella, scritta in Visual C# e Visual Basic, che consente di dividere una stringa delimitata da virgole in una tabella con una colonna,</span><span class="sxs-lookup"><span data-stu-id="e750c-103">The String Utilities sample contains a streaming table-valued function written in Visual C# and Visual Basic, which splits a comma-separated string into a table with one column.</span></span> <span data-ttu-id="e750c-104">nonché una funzione di aggregazione che consente di convertire una colonna stringa in una stringa delimitata da virgole.</span><span class="sxs-lookup"><span data-stu-id="e750c-104">It also contains an aggregate function that converts a string column to a comma-separated string.</span></span>  <span data-ttu-id="e750c-105">Vengono inoltre implementate una funzione scalare e una funzione con valori di tabella che garantiscono funzionalità di ricerca e sostituzione di espressioni regolari.</span><span class="sxs-lookup"><span data-stu-id="e750c-105">Also implemented are a scalar function and a table-valued function that provide regular expression replacement and regular expression searching functionality.</span></span>  
  
 <span data-ttu-id="e750c-106">Per implementare una funzione di flusso con valori di tabella, creare un metodo per la restituzione di un oggetto che implementi l'interfaccia `IEnumerable`.</span><span class="sxs-lookup"><span data-stu-id="e750c-106">To implement a streaming table-valued function, create a method that returns an object that implements the `IEnumerable` interface.</span></span> <span data-ttu-id="e750c-107">Il metodo `IEnumerable` deve essere collegato tramite un attributo a un altro metodo per la compilazione delle righe della funzione con valori di tabella.</span><span class="sxs-lookup"><span data-stu-id="e750c-107">This `IEnumerable` method must be linked by an attribute to another method which fills in rows of the table-valued function.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="e750c-108">Prerequisiti</span><span class="sxs-lookup"><span data-stu-id="e750c-108">Prerequisites</span></span>  
 <span data-ttu-id="e750c-109">Per creare ed eseguire questo progetto, è necessario installare il software seguente:</span><span class="sxs-lookup"><span data-stu-id="e750c-109">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="e750c-110">o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="e750c-110">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="e750c-111">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express è disponibile gratuitamente nel [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sito Web [di documentazione ed esempi di](https://www.microsoft.com/download/details.aspx?id=42299)Express</span><span class="sxs-lookup"><span data-stu-id="e750c-111">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/download/details.aspx?id=42299)</span></span>  
  
-   <span data-ttu-id="e750c-112">Database AdventureWorks, disponibile nel [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sito Web [per sviluppatori di](https://archive.codeplex.com/?p=SqlServerSamples)</span><span class="sxs-lookup"><span data-stu-id="e750c-112">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://archive.codeplex.com/?p=SqlServerSamples)</span></span>  
  
-   <span data-ttu-id="e750c-113">.NET Framework SDK 2.0 o versione successiva oppure Microsoft Visual Studio 2005 o versione successiva.</span><span class="sxs-lookup"><span data-stu-id="e750c-113">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="e750c-114">.NET Framework SDK è disponibile gratuitamente.</span><span class="sxs-lookup"><span data-stu-id="e750c-114">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="e750c-115">È necessario inoltre che siano soddisfatte le condizioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="e750c-115">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="e750c-116">Per l'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] usata deve essere abilitata l'integrazione con CLR.</span><span class="sxs-lookup"><span data-stu-id="e750c-116">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="e750c-117">Per abilitare l'integrazione con CLR, effettuare le operazioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="e750c-117">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="e750c-118">Abilitazione dell'integrazione con CLR</span><span class="sxs-lookup"><span data-stu-id="e750c-118">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="e750c-119">Eseguire i comandi [!INCLUDE[tsql](../../includes/tsql-md.md)] seguenti:</span><span class="sxs-lookup"><span data-stu-id="e750c-119">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="e750c-120">Per abilitare l'integrazione con CLR, è necessario disporre dell'autorizzazione `ALTER SETTINGS` a livello di server, che viene assegnata implicitamente ai membri dei ruoli predefiniti del server `sysadmin` e `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="e750c-120">To enable CLR integration, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="e750c-121">Il database AdventureWorks deve essere installato nell'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] in uso.</span><span class="sxs-lookup"><span data-stu-id="e750c-121">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="e750c-122">Se non si è un amministratore per l' [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] istanza di in uso, è necessario disporre di un amministratore per concedere l'autorizzazione **CreateAssembly** per completare l'installazione.</span><span class="sxs-lookup"><span data-stu-id="e750c-122">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="e750c-123">Compilazione dell'esempio</span><span class="sxs-lookup"><span data-stu-id="e750c-123">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="e750c-124">Creare ed eseguire l'esempio tramite le istruzioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="e750c-124">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="e750c-125">Aprire un prompt dei comandi di .NET Framework o Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="e750c-125">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="e750c-126">Se necessario, creare una directory per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="e750c-126">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="e750c-127">Per questo esempio verrà utilizzata la directory C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="e750c-127">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="e750c-128">In c:\MySample creare `StringUtils.vb` (per l'esempio Visual Basic) o `StringUtils.cs` (per l'esempio C#) e copiare nel file il codice di esempio appropriato, Visual Basic o C#, riportato di seguito.</span><span class="sxs-lookup"><span data-stu-id="e750c-128">In c:\MySample, create `StringUtils.vb` (for the Visual Basic sample) or `StringUtils.cs` (for the C# sample) and copy the appropriate Visual Basic or C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="e750c-129">Compilare il codice di esempio dal prompt della riga di comando eseguendo una delle istruzioni seguenti, a seconda del linguaggio scelto.</span><span class="sxs-lookup"><span data-stu-id="e750c-129">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
5.  <span data-ttu-id="e750c-130">Copiare il codice di installazione [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `Install.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="e750c-130">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
6.  <span data-ttu-id="e750c-131">Se l'esempio è installato in una directory diversa da `C:\MySample\`, modificare il file `Install.sql` come indicato, in modo che punti al percorso appropriato.</span><span class="sxs-lookup"><span data-stu-id="e750c-131">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
7.  <span data-ttu-id="e750c-132">Distribuire l'assembly e la stored procedure eseguendo</span><span class="sxs-lookup"><span data-stu-id="e750c-132">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
    -   <span data-ttu-id="e750c-133">Copiare lo script di comandi di test [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `test.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="e750c-133">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
8.  <span data-ttu-id="e750c-134">Eseguire lo script di test con il comando seguente</span><span class="sxs-lookup"><span data-stu-id="e750c-134">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
9. <span data-ttu-id="e750c-135">Copiare lo script di pulizia [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `cleanup.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="e750c-135">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
10. <span data-ttu-id="e750c-136">Eseguire lo script con il comando seguente</span><span class="sxs-lookup"><span data-stu-id="e750c-136">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
  
## <a name="sample-code"></a><span data-ttu-id="e750c-137">Codice di esempio</span><span class="sxs-lookup"><span data-stu-id="e750c-137">Sample Code</span></span>  
 <span data-ttu-id="e750c-138">Di seguito sono illustrati i listati di codice per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="e750c-138">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="e750c-139">C#</span><span class="sxs-lookup"><span data-stu-id="e750c-139">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="e750c-140">VB.NET</span><span class="sxs-lookup"><span data-stu-id="e750c-140">VB.NET</span></span>  
...
```  
 ...
 <span data-ttu-id="e750c-141">Di seguito è illustrato lo script di installazione [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) che consente la distribuzione dell'assembly e la creazione della stored procedure nel database.</span><span class="sxs-lookup"><span data-stu-id="e750c-141">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the stored procedure in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="e750c-142">Di seguito è illustrato lo script `test.sql`che consente di testare l'esempio eseguendo le funzioni.</span><span class="sxs-lookup"><span data-stu-id="e750c-142">This is `test.sql`, which tests the sample by executing the functions.</span></span>  
...
```  
 ...
 <span data-ttu-id="e750c-143">Il codice [!INCLUDE[tsql](../../includes/tsql-md.md)] seguente consente di rimuovere l'assembly e le funzioni dal database.</span><span class="sxs-lookup"><span data-stu-id="e750c-143">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and functions from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="e750c-144">Vedere anche</span><span class="sxs-lookup"><span data-stu-id="e750c-144">See Also</span></span>  
 [<span data-ttu-id="e750c-145">Scenari di utilizzo ed esempi per l'integrazione con CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="e750c-145">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
