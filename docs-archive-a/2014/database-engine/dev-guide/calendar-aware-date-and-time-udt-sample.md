---
title: Esempio di data e ora in grado di riconoscere il calendario | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "87626250"
---
# <a name="calendar-aware-date-and-time-udt-sample"></a><span data-ttu-id="44a47-102">Esempio di data e ora in grado di riconoscere il calendario</span><span class="sxs-lookup"><span data-stu-id="44a47-102">Calendar-Aware Date and Time UDT Sample</span></span>
  <span data-ttu-id="44a47-103">L'archiviazione di date sotto forma di stringhe può provocare confusione in quanto le date non hanno alcun significato se non si conosce il sistema di calendario utilizzato. Nell'esempio `CADatetime` vengono illustrati due tipi di dati definiti dall'utente, `CADatetime` e `CADate`, che consentono la gestione di data e ora con supporto del calendario.</span><span class="sxs-lookup"><span data-stu-id="44a47-103">Storing dates as strings can be confusing because dates are meaningless without understanding what calendar system is being used.The `CADatetime` sample defines two user-defined data types, `CADatetime` and `CADate`, which provide calendar-aware handling of dates and times.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="44a47-104">Prerequisiti</span><span class="sxs-lookup"><span data-stu-id="44a47-104">Prerequisites</span></span>  
 <span data-ttu-id="44a47-105">Per creare ed eseguire questo progetto, è necessario installare il software seguente:</span><span class="sxs-lookup"><span data-stu-id="44a47-105">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="44a47-106">o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="44a47-106">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="44a47-107">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express è disponibile gratuitamente nel [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sito Web [di documentazione ed esempi di](https://www.microsoft.com/sql-server/sql-server-editions-express)Express</span><span class="sxs-lookup"><span data-stu-id="44a47-107">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="44a47-108">Database AdventureWorks, disponibile nel [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sito Web [per sviluppatori di](https://go.microsoft.com/fwlink/?linkid=62796)</span><span class="sxs-lookup"><span data-stu-id="44a47-108">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="44a47-109">.NET Framework SDK 2.0 o versione successiva oppure Microsoft Visual Studio 2005 o versione successiva.</span><span class="sxs-lookup"><span data-stu-id="44a47-109">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="44a47-110">.NET Framework SDK è disponibile gratuitamente.</span><span class="sxs-lookup"><span data-stu-id="44a47-110">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="44a47-111">È necessario inoltre che siano soddisfatte le condizioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="44a47-111">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="44a47-112">Per l'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] usata deve essere abilitata l'integrazione con CLR.</span><span class="sxs-lookup"><span data-stu-id="44a47-112">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="44a47-113">Per abilitare l'integrazione con CLR, effettuare le operazioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="44a47-113">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="44a47-114">Abilitazione dell'integrazione con CLR</span><span class="sxs-lookup"><span data-stu-id="44a47-114">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="44a47-115">Eseguire i comandi [!INCLUDE[tsql](../../includes/tsql-md.md)] seguenti:</span><span class="sxs-lookup"><span data-stu-id="44a47-115">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="44a47-116">Per abilitare CLR, è necessario disporre dell'autorizzazione `ALTER SETTINGS` a livello di server, che viene assegnata implicitamente ai membri dei ruoli predefiniti del server `sysadmin` e `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="44a47-116">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="44a47-117">Il database AdventureWorks deve essere installato nell'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] in uso.</span><span class="sxs-lookup"><span data-stu-id="44a47-117">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="44a47-118">Se non si è un amministratore per l' [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] istanza di in uso, è necessario disporre di un amministratore per concedere l'autorizzazione **CreateAssembly** per completare l'installazione.</span><span class="sxs-lookup"><span data-stu-id="44a47-118">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="44a47-119">Compilazione dell'esempio</span><span class="sxs-lookup"><span data-stu-id="44a47-119">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="44a47-120">Creare ed eseguire l'esempio tramite le istruzioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="44a47-120">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="44a47-121">Aprire un prompt dei comandi di .NET Framework o Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="44a47-121">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="44a47-122">Se necessario, creare una directory per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="44a47-122">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="44a47-123">Per questo esempio verrà utilizzata la directory C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="44a47-123">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="44a47-124">In c:\MySample creare `CalendarAware.cs` e copiare il codice C# di esempio, riportato di seguito, nel file.</span><span class="sxs-lookup"><span data-stu-id="44a47-124">In c:\MySample, create `CalendarAware.cs` and copy the C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="44a47-125">In c:\MySample creare il file `calendars.txt` e copiare il codice di esempio nel file.</span><span class="sxs-lookup"><span data-stu-id="44a47-125">In c:\MySample, create the file `calendars.txt` and copy the sample code into the file.</span></span>  
  
5.  <span data-ttu-id="44a47-126">In c:\MySample creare il file `calendars.ar-SA.txt` e copiare il codice seguente nel file:</span><span class="sxs-lookup"><span data-stu-id="44a47-126">In c:\MySample, create the file `calendars.ar-SA.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 1`  
  
6.  <span data-ttu-id="44a47-127">In c:\MySample creare il file `calendars.ja.txt` e copiare il codice seguente nel file:</span><span class="sxs-lookup"><span data-stu-id="44a47-127">In c:\MySample, create the file `calendars.ja.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 3`  
  
7.  <span data-ttu-id="44a47-128">In c:\MySample creare il file `calendars.``zh-CN.txt` e copiare il codice seguente nel file:</span><span class="sxs-lookup"><span data-stu-id="44a47-128">In c:\MySample, create the file `calendars.``zh-CN.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 8`  
  
8.  <span data-ttu-id="44a47-129">In c:\MySample creare il file `build.com` e copiare il codice seguente nel file:</span><span class="sxs-lookup"><span data-stu-id="44a47-129">In c:\MySample, create the file `build.com` and copy the following into the file:</span></span>  
...
9. <span data-ttu-id="44a47-130">Compilare l'assembly satellite eseguendo la compilazione del file al prompt dei comandi.</span><span class="sxs-lookup"><span data-stu-id="44a47-130">Build the satellite assembles by executing the file build at the command prompt.</span></span>  
  
10. <span data-ttu-id="44a47-131">Compilare il codice di esempio dal prompt della riga di comando eseguendo l'istruzione seguente:</span><span class="sxs-lookup"><span data-stu-id="44a47-131">Compile the sample code from the command line prompt by executing the following:</span></span>  
...
11. <span data-ttu-id="44a47-132">Copiare il codice di installazione [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `Install.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="44a47-132">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
12. <span data-ttu-id="44a47-133">Se l'esempio è installato in una directory diversa da `C:\MySample\`, modificare il file `Install.sql` come indicato, in modo che punti al percorso appropriato.</span><span class="sxs-lookup"><span data-stu-id="44a47-133">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
13. <span data-ttu-id="44a47-134">Distribuire l'assembly e la stored procedure eseguendo</span><span class="sxs-lookup"><span data-stu-id="44a47-134">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
14. <span data-ttu-id="44a47-135">Copiare lo script di comandi di test [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `test.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="44a47-135">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
15. <span data-ttu-id="44a47-136">Eseguire lo script di test con il comando seguente</span><span class="sxs-lookup"><span data-stu-id="44a47-136">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
16. <span data-ttu-id="44a47-137">Copiare lo script di pulizia [!INCLUDE[tsql](../../includes/tsql-md.md)] in un file e salvarlo come `cleanup.sql` nella directory dell'esempio.</span><span class="sxs-lookup"><span data-stu-id="44a47-137">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
17. <span data-ttu-id="44a47-138">Eseguire lo script con il comando seguente</span><span class="sxs-lookup"><span data-stu-id="44a47-138">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="44a47-139">Codice di esempio</span><span class="sxs-lookup"><span data-stu-id="44a47-139">Sample Code</span></span>  
 <span data-ttu-id="44a47-140">Di seguito sono illustrati i listati di codice per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="44a47-140">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="44a47-141">C#</span><span class="sxs-lookup"><span data-stu-id="44a47-141">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="44a47-142">Di seguito è illustrato il file `calendars.txt` necessario per la definizione dei calendari.</span><span class="sxs-lookup"><span data-stu-id="44a47-142">This is the file `calendars.txt` needed for defining the calendars.</span></span>  
...
```  
 ...
 <span data-ttu-id="44a47-143">Di seguito è illustrato lo script di installazione [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) che consente la distribuzione degli assembly e la creazione delle funzioni di conversione nel database, nonché la preparazione di una tabella di dati di esempio.</span><span class="sxs-lookup"><span data-stu-id="44a47-143">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assemblies and creates the conversion functions within the database; it also crates a table of sample data.</span></span>  
...
```  
 ...
 <span data-ttu-id="44a47-144">Di seguito è illustrato lo script `test.sql` che consente di testare l'esempio eseguendo le funzioni in calendari diversi.</span><span class="sxs-lookup"><span data-stu-id="44a47-144">This is `test.sql`, which tests the sample by executing the functions on various calendars.</span></span>  
...
```  
 ...
 <span data-ttu-id="44a47-145">Il codice [!INCLUDE[tsql](../../includes/tsql-md.md)] seguente consente di rimuovere gli assembly, le funzioni e la tabella dal database.</span><span class="sxs-lookup"><span data-stu-id="44a47-145">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assemblies, functions and table from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="44a47-146">Vedere anche</span><span class="sxs-lookup"><span data-stu-id="44a47-146">See Also</span></span>  
 [<span data-ttu-id="44a47-147">Scenari di utilizzo ed esempi per l'integrazione con CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="44a47-147">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
