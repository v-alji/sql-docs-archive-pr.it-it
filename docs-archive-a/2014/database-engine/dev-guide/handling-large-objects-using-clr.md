...
ms.locfileid: "87726192"
---
# <a name="handling-large-objects-using-clr"></a><span data-ttu-id="bb584-102">Gestione di oggetti di grandi dimensioni tramite CLR</span><span class="sxs-lookup"><span data-stu-id="bb584-102">Handling Large Objects Using CLR</span></span>
  <span data-ttu-id="bb584-103">Nell'esempio `HandlingLOBUsingCLR` relativo a SQL Server viene illustrato il trasferimento di oggetti di grandi dimensioni (LOB) tra [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] e un file system accessibile dal server tramite stored procedure Common Language Runtime (CLR).</span><span class="sxs-lookup"><span data-stu-id="bb584-103">The `HandlingLOBUsingCLR` sample for SQL Server demonstrates transferring large objects (LOBs) between [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] and a file system that is available to the server by using common language runtime (CLR) stored procedures.</span></span> <span data-ttu-id="bb584-104">In questo esempio viene illustrata la procedura di accesso ai file nel codice sul lato server e quindi la procedura di richiamo di query dinamiche e di stored procedure dalle stored procedure basate su CLR.</span><span class="sxs-lookup"><span data-stu-id="bb584-104">This sample demonstrates how to access files in server-side code, and then invoke both dynamic queries and stored procedures from CLR-based stored procedures.</span></span> <span data-ttu-id="bb584-105">Viene inoltre descritta la procedura di registrazione e annullamento della registrazione di metodi e assembly CLR tramite [!INCLUDE[tsql](../../includes/tsql-md.md)].</span><span class="sxs-lookup"><span data-stu-id="bb584-105">It also demonstrates how to unregister and register CLR methods and assemblies by using [!INCLUDE[tsql](../../includes/tsql-md.md)].</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="bb584-106">Prerequisiti</span><span class="sxs-lookup"><span data-stu-id="bb584-106">Prerequisites</span></span>  
 <span data-ttu-id="bb584-107">Per creare ed eseguire questo progetto, è necessario installare il software seguente:</span><span class="sxs-lookup"><span data-stu-id="bb584-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="bb584-108">o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="bb584-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="bb584-109">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express è disponibile gratuitamente nel [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sito Web [di documentazione ed esempi di](https://www.microsoft.com/download/details.aspx?id=42299)Express</span><span class="sxs-lookup"><span data-stu-id="bb584-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/download/details.aspx?id=42299)</span></span>  
  
-   <span data-ttu-id="bb584-110">Database AdventureWorks, disponibile nel [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sito Web [per sviluppatori di](https://archive.codeplex.com/?p=SqlServerSamples)</span><span class="sxs-lookup"><span data-stu-id="bb584-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://archive.codeplex.com/?p=SqlServerSamples)</span></span>  
  
-   <span data-ttu-id="bb584-111">.NET Framework SDK 2.0 o versione successiva oppure Microsoft Visual Studio 2005 o versione successiva.</span><span class="sxs-lookup"><span data-stu-id="bb584-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="bb584-112">.NET Framework SDK è disponibile gratuitamente.</span><span class="sxs-lookup"><span data-stu-id="bb584-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="bb584-113">È necessario inoltre che siano soddisfatte le condizioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="bb584-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="bb584-114">Per l'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] usata deve essere abilitata l'integrazione con CLR.</span><span class="sxs-lookup"><span data-stu-id="bb584-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="bb584-115">Per abilitare l'integrazione con CLR, effettuare le operazioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="bb584-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="bb584-116">Abilitazione dell'integrazione con CLR</span><span class="sxs-lookup"><span data-stu-id="bb584-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="bb584-117">Eseguire i comandi [!INCLUDE[tsql](../../includes/tsql-md.md)] seguenti:</span><span class="sxs-lookup"><span data-stu-id="bb584-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="bb584-118">Per abilitare CLR, è necessario disporre dell'autorizzazione `ALTER SETTINGS` a livello di server, che viene assegnata implicitamente ai membri dei ruoli predefiniti del server `sysadmin` e `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="bb584-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="bb584-119">Il database AdventureWorks deve essere installato nell'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] in uso.</span><span class="sxs-lookup"><span data-stu-id="bb584-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="bb584-120">Se non si è un amministratore per l' [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] istanza di in uso, è necessario disporre di un amministratore per concedere l'autorizzazione **CreateAssembly** per completare l'installazione.</span><span class="sxs-lookup"><span data-stu-id="bb584-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="bb584-121">Compilazione dell'esempio</span><span class="sxs-lookup"><span data-stu-id="bb584-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="bb584-122">Creare ed eseguire l'esempio tramite le istruzioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="bb584-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="bb584-123">Aprire un prompt dei comandi di .NET Framework o Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="bb584-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="bb584-124">Se necessario, creare una directory per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="bb584-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="bb584-125">Per questo esempio verrà utilizzata la directory C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="bb584-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="bb584-126">Poiché per questo esempio è necessario un assembly firmato, creare una chiave asimmetrica digitando il comando:</span><span class="sxs-lookup"><span data-stu-id="bb584-126">Since this example requires a signed assembly, create an asymmetric key by typing the command:</span></span>  
  
## <a name="sample-code"></a><span data-ttu-id="bb584-127">Codice di esempio</span><span class="sxs-lookup"><span data-stu-id="bb584-127">Sample Code</span></span>  
 <span data-ttu-id="bb584-128">Di seguito sono illustrati i listati di codice per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="bb584-128">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="bb584-129">C#</span><span class="sxs-lookup"><span data-stu-id="bb584-129">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="bb584-130">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="bb584-130">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="bb584-131">Questo script di installazione [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) consente la distribuzione dell'assembly e la creazione delle stored procedure e degli oggetti di sicurezza necessari per l'esempio.</span><span class="sxs-lookup"><span data-stu-id="bb584-131">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the stored procedures and security objects required by this example.</span></span>  
...
```  
 ...
 <span data-ttu-id="bb584-132">Il codice seguente [!INCLUDE[tsql](../../includes/tsql-md.md)] ( `test.sql` ) consente di testare l'esempio esercitando le stored procedure.</span><span class="sxs-lookup"><span data-stu-id="bb584-132">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] (`test.sql`) tests the example by exercising the stored procedures.</span></span>  
...
```  
 ...
 <span data-ttu-id="bb584-133">Il codice [!INCLUDE[tsql](../../includes/tsql-md.md)] seguente consente di rimuovere l'assembly, l'account di accesso, la chiave e le stored procedure dal database.</span><span class="sxs-lookup"><span data-stu-id="bb584-133">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly, login, key and stored procedures from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="bb584-134">Vedere anche</span><span class="sxs-lookup"><span data-stu-id="bb584-134">See Also</span></span>  
 [<span data-ttu-id="bb584-135">Scenari di utilizzo ed esempi per l'integrazione con CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="bb584-135">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
