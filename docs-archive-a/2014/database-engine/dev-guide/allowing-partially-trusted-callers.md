---
title: Consentire chiamanti parzialmente attendibili | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87626257"
---
# <a name="allowing-partially-trusted-callers"></a><span data-ttu-id="30524-102">Accettazione di chiamanti parzialmente attendibili</span><span class="sxs-lookup"><span data-stu-id="30524-102">Allowing Partially Trusted Callers</span></span>
  <span data-ttu-id="30524-103">La condivisione delle librerie di codice rappresenta uno scenario comune quando si utilizza l'integrazione con Common Language Runtime (CLR), in cui a un assembly che contiene un tipo definito dall'utente, una stored procedure, una funzione definita dall'utente, un'aggregazione definita dall'utente, un trigger o una classe di utilità spesso accede un altro assembly o applicazione.</span><span class="sxs-lookup"><span data-stu-id="30524-103">Sharing code libraries is a common scenario with common language runtime (CLR) integration, where an assembly containing a user-defined type, stored procedure, user-defined function, user-defined aggregate, trigger, or utility class is often accessed by another assembly or application.</span></span> <span data-ttu-id="30524-104">Le librerie di codice per le quali è prevista la condivisione da parte di più applicazioni devono essere firmate con un nome sicuro.</span><span class="sxs-lookup"><span data-stu-id="30524-104">Code libraries that are to be shared by multiple applications must be signed with a strong name.</span></span>  
  
 <span data-ttu-id="30524-105">Solo le applicazioni considerate completamente attendibili dal sistema di sicurezza da accesso di codice runtime possono accedere a un assembly del codice gestito condiviso non contrassegnato in modo esplicito dall'attributo `System.Security.AllowPartiallyTrustedCallers`.</span><span class="sxs-lookup"><span data-stu-id="30524-105">Only applications that are fully trusted by the runtime code access security system are allowed to access a shared managed code assembly that is not explicitly marked with the `System.Security.AllowPartiallyTrustedCallers` attribute.</span></span> <span data-ttu-id="30524-106">Un assembly parzialmente attendibile, ovvero registrato in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] con il set di autorizzazioni `SAFE` o `EXTERNAL_ACCESS`, che tenta di accedere a un assembly firmato con nome sicuro senza questo attributo determina la generazione di una `System.Security.SecurityException`.</span><span class="sxs-lookup"><span data-stu-id="30524-106">A partially trusted assembly (one that is registered in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] with the `SAFE` or `EXTERNAL_ACCESS` permission set) that attempts to access a strong-name signed assembly without this attribute causes a `System.Security.SecurityException` to be thrown.</span></span> <span data-ttu-id="30524-107">Il messaggio di errore visualizzato è simile al seguente:</span><span class="sxs-lookup"><span data-stu-id="30524-107">The error message you see is similar to the following:</span></span>  
...
```  
 ...
 <span data-ttu-id="30524-108">È consigliabile contrassegnare tutti gli assembly registrati in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], ad eccezione di quelli aggiunti alla Global Assembly Cache, con l'attributo `AllowPartiallyTrustedCallers` in modo che gli assembly caricati da [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] possono accedere l'un all'altro.</span><span class="sxs-lookup"><span data-stu-id="30524-108">We recommend that all assemblies registered in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], except those assemblies added to the global assembly cache, be marked with the `AllowPartiallyTrustedCallers` attribute so that assemblies loaded by [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] can access each other.</span></span> <span data-ttu-id="30524-109">È importante sottoporre gli assembly da aggiungere alla Global Assembly Cache ad un accurato controllo di sicurezza prima dell'aggiunta dell'attributo `AllowPartiallyTrustedCallers`, in quanto l'assembly diventerebbe quindi disponibile ai chiamanti parzialmente attendibili da contesti imprevisti.</span><span class="sxs-lookup"><span data-stu-id="30524-109">Assemblies that are to be added to the global assembly cache should be thoroughly reviewed for safety before adding the `AllowPartiallyTrustedCallers` attribute, as the assembly would then be available to partially trusted callers from unexpected contexts.</span></span> <span data-ttu-id="30524-110">Un assembly non deve essere mai reso completamente attendibile, ovvero registrato con il set di autorizzazioni `UNSAFE` in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].</span><span class="sxs-lookup"><span data-stu-id="30524-110">An assembly should not be made fully trusted (registered with the `UNSAFE` permission set in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]).</span></span>  
  
 <span data-ttu-id="30524-111">Per ulteriori informazioni, vedere la sezione relativa all'utilizzo delle librerie da codice parzialmente attendibile nel Software Development Kit di .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="30524-111">For more information, see the "Using Libraries from Partially Trusted Code" section in the .NET Framework software development kit.</span></span>  
  
## <a name="example"></a><span data-ttu-id="30524-112">Esempio</span><span class="sxs-lookup"><span data-stu-id="30524-112">Example</span></span>  
  
### <a name="description"></a><span data-ttu-id="30524-113">Descrizione</span><span class="sxs-lookup"><span data-stu-id="30524-113">Description</span></span>  
 <span data-ttu-id="30524-114">Si supponga di disporre di una classe di utilità che risulta utile per molte applicazioni di integrazione con CLR sul lato server,</span><span class="sxs-lookup"><span data-stu-id="30524-114">Suppose there is a utility class that would be useful for many server-side CLR integration applications.</span></span> <span data-ttu-id="30524-115">ad esempio una classe che rappresenta i risultati della chiamata a una query.</span><span class="sxs-lookup"><span data-stu-id="30524-115">For example, it might be a class that represents the results of invoking a query.</span></span> <span data-ttu-id="30524-116">Per abilitare la condivisione di questo componente, questa classe di utilità viene inserita in un assembly separato a</span><span class="sxs-lookup"><span data-stu-id="30524-116">To enable sharing of this component, this utility class is placed in a separate assembly.</span></span> <span data-ttu-id="30524-117">cui viene quindi fatto riferimento dai vari altri assembly che contengono oggetti dell'integrazione con CLR.</span><span class="sxs-lookup"><span data-stu-id="30524-117">Then that assembly is referenced from various other assemblies that contain CLR integration objects.</span></span> <span data-ttu-id="30524-118">Poiché questa classe di utilità è utilizzata in molte applicazioni server diverse, essa viene sottoposta a un'attenta revisione e gli eventuali problemi legati alla sicurezza vengono risolti.</span><span class="sxs-lookup"><span data-stu-id="30524-118">Because this utility class is used in many different server applications, it is reviewed carefully and any security issues are resolved.</span></span> <span data-ttu-id="30524-119">L'attributo `AllowPartiallyTrustedCallers` viene quindi applicato all'assembly che contiene la classe di utilità, in modo che gli oggetti dell'integrazione con CLR contenuti negli assembly contrassegnati con i set di autorizzazioni `SAFE` o `EXTERNAL_ACCESS` possano utilizzare i metodi e la classe di utilità anche se si trovano in un assembly separato.</span><span class="sxs-lookup"><span data-stu-id="30524-119">The `AllowPartiallyTrustedCallers` attribute is then applied to the assembly that contains the utility class, so that CLR integration objects contained in assemblies marked with `SAFE` or `EXTERNAL_ACCESS` permission sets can use the utility class and methods, even though they are in a separate assembly.</span></span>  
  
 <span data-ttu-id="30524-120">A volte può rivelarsi utile poter eseguire comandi mentre si stanno leggendo i risultati di una query senza dover aprire una nuova connessione e senza dover leggere tutti i risultati in memoria.</span><span class="sxs-lookup"><span data-stu-id="30524-120">Sometimes it is useful to be able to execute commands while reading through results of a query without opening a new connection and without reading all the results into memory.</span></span> <span data-ttu-id="30524-121">La caratteristica Multiple Active Result Set (MARS) di ADO.NET 2.0 è una tecnologia che consente di raggiungere tale obiettivo.</span><span class="sxs-lookup"><span data-stu-id="30524-121">The Multiple Active Result Set (MARS) feature in ADO.NET 2.0 is a technology that can help you achieve that.</span></span> <span data-ttu-id="30524-122">Attualmente questa funzionalità non è implementata per il provider in-process utilizzato nella programmazione sul lato server.</span><span class="sxs-lookup"><span data-stu-id="30524-122">Currently, MARS is not implemented for the in-process provider used for server-side programming.</span></span> <span data-ttu-id="30524-123">Per ovviare a questa limitazione è possibile utilizzare cursori sul lato server.</span><span class="sxs-lookup"><span data-stu-id="30524-123">To work around this limitation, you can use server-side cursors.</span></span> <span data-ttu-id="30524-124">Questo esempio illustra come utilizzare i cursori sul lato server per ovviare alla mancanza del supporto MARS nella programmazione sul lato server.</span><span class="sxs-lookup"><span data-stu-id="30524-124">This sample demonstrates how to use server-side cursors to work around the lack of MARS support for server-side programming.</span></span>  
  
 <span data-ttu-id="30524-125">L'utilizzo di cursori sul lato server comporta un impegno significativo delle risorse del server e talvolta impedisce a Query Optimizer in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] di migliorare le prestazioni delle query.</span><span class="sxs-lookup"><span data-stu-id="30524-125">Using server-side cursors is expensive in terms of server resources and can sometimes prevent the query optimizer in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] from enhancing the performance of your queries.</span></span> <span data-ttu-id="30524-126">È pertanto consigliabile riscrivere il codice per utilizzare istruzioni JOIN, se possibile.</span><span class="sxs-lookup"><span data-stu-id="30524-126">Therefore, you might consider rewriting your code to use JOINs whenever possible.</span></span>  
  
 <span data-ttu-id="30524-127">L'API per questa classe è simile a un lettore di dati, ad eccezione del fatto che è possibile passare al risultato precedente o successivo nel set di risultati ed eseguire altri comandi sulla connessione mentre il set di risultati è aperto.</span><span class="sxs-lookup"><span data-stu-id="30524-127">The API for this class is similar to a data reader except that you can move forward or backward throughout the result set, and other commands can be issued on the connection while the result set is open.</span></span>  
  
 <span data-ttu-id="30524-128">Per facilitare la comprensione dell'esempio, questa implementazione è stata notevolmente semplificata.</span><span class="sxs-lookup"><span data-stu-id="30524-128">This implementation is highly simplified to make the sample easy to understand.</span></span> <span data-ttu-id="30524-129">In un'implementazione più efficace verrebbero recuperate più righe per evitare turnaround del database per ogni riga recuperata.</span><span class="sxs-lookup"><span data-stu-id="30524-129">A more efficient implementation would fetch multiple rows to avoid a database turnaround per row fetched.</span></span>  
  
 <span data-ttu-id="30524-130">L'utilizzo di questa classe garantisce un footprint di memoria notevolmente inferiore rispetto a quello derivante dall'inserimento di tutti i risultati della query in un set di dati, aspetto di estrema importanza nella programmazione sul lato server.</span><span class="sxs-lookup"><span data-stu-id="30524-130">Using this class can have a significantly smaller memory footprint than filling a dataset with all the results of a query, which is very important for server-side programming.</span></span>  
  
 <span data-ttu-id="30524-131">L'esempio illustra inoltre l'utilizzo dell'attributo "Allow partially trusted callers" per indicare che l'assembly del set di risultati è una libreria che può essere chiamata da altri assembly senza problemi.</span><span class="sxs-lookup"><span data-stu-id="30524-131">This sample also demonstrates using the "Allow partially trusted callers" attribute to indicate that the Result Set assembly is a library which may be safely called from other assemblies.</span></span> <span data-ttu-id="30524-132">Questo approccio è leggermente più complesso ma molto più sicuro rispetto alla registrazione dell'assembly chiamante tramite l'autorizzazione UNSAFE.</span><span class="sxs-lookup"><span data-stu-id="30524-132">This approach is a little more complex but much more secure than registering the calling assembly using the unsafe permission.</span></span> <span data-ttu-id="30524-133">È più sicuro in quanto, registrando l'assembly chiamante come sicuro, si limitano le risorse interessate al di fuori del server e si evitano danni all'integrità del server.</span><span class="sxs-lookup"><span data-stu-id="30524-133">It is more secure because registering the calling assembly as safe the calling assembly limits affecting resources off the server and prevents damage to the integrity of the server.</span></span>  
  
 <span data-ttu-id="30524-134">Le istruzioni di compilazione per questo esempio presuppongono che i file di codice sorgente si trovino in una directory denominata c:\samples.</span><span class="sxs-lookup"><span data-stu-id="30524-134">The build instructions for this sample assume that the source code files are in a directory called c:\samples.</span></span>  <span data-ttu-id="30524-135">Se si utilizza un directory diversa, è necessario modificare gli script [!INCLUDE[tsql](../../includes/tsql-md.md)].</span><span class="sxs-lookup"><span data-stu-id="30524-135">If you use another directory, you will have to modify the [!INCLUDE[tsql](../../includes/tsql-md.md)] scripts.</span></span> <span data-ttu-id="30524-136">Gli [!INCLUDE[tsql](../../includes/tsql-md.md)] script richiedono inoltre il database AdventureWorks.</span><span class="sxs-lookup"><span data-stu-id="30524-136">The [!INCLUDE[tsql](../../includes/tsql-md.md)] scripts also requires the AdventureWorks database.</span></span> <span data-ttu-id="30524-137">È possibile scaricare il database di esempio AdventureWorks dal [Microsoft SQL Server esempi e progetti della Community](https://go.microsoft.com/fwlink/?LinkID=85384) Home page.</span><span class="sxs-lookup"><span data-stu-id="30524-137">You can download the AdventureWorks sample database from the [Microsoft SQL Server Samples and Community Projects](https://go.microsoft.com/fwlink/?LinkID=85384) home page.</span></span>  
  
 <span data-ttu-id="30524-138">Per compilare ed eseguire l'esempio, incollare il primo listato di codice in un file denominato ResultSet.cs ed eseguire la compilazione con csc /target:library ResultSet.cs.</span><span class="sxs-lookup"><span data-stu-id="30524-138">To build and run the sample, paste the first code listing into a file called ResultSet.cs and compile with csc /target:library ResultSet.cs.</span></span>  
  
 <span data-ttu-id="30524-139">Incollare il secondo listato di codice in un file denominato TestResultSet.cs ed eseguire la compilazione con csc /target:library /reference:ResultSet.dll TestResultSet.cs.</span><span class="sxs-lookup"><span data-stu-id="30524-139">Paste the second code listing into a file called TestResultSet.cs and compile with csc /target:library /reference:ResultSet.dll TestResultSet.cs.</span></span>  
  
 <span data-ttu-id="30524-140">Incollare il terzo listato di codice in un file denominato InstallCS.sql ed eseguire la compilazione con sqlcmd -E -I -i InstallCS.sql.</span><span class="sxs-lookup"><span data-stu-id="30524-140">Paste the third code listing into a file called InstallCS.sql and compile with sqlcmd -E -I -i InstallCS.sql.</span></span>  
  
 <span data-ttu-id="30524-141">Incollare il quarto listato di codice in un file denominato test.sql ed eseguire la compilazione con sqlcmd -E -I -i test.sql.</span><span class="sxs-lookup"><span data-stu-id="30524-141">Paste the fourth code listing into a file called test.sql  and compile with sqlcmd -E -I -i test.sql.</span></span>  
  
 <span data-ttu-id="30524-142">Incollare il quinto listato di codice in un file denominato Cleanup.sql ed eseguire la compilazione con sqlcmd -E -I -i Cleanup.sql.</span><span class="sxs-lookup"><span data-stu-id="30524-142">Paste the fifth code listing into a file called Cleanup.sql  and compile with sqlcmd -E -I -i Cleanup.sql.</span></span>  
  
### <a name="code"></a><span data-ttu-id="30524-143">Codice</span><span class="sxs-lookup"><span data-stu-id="30524-143">Code</span></span>  
...
### <a name="code"></a><span data-ttu-id="30524-144">Codice</span><span class="sxs-lookup"><span data-stu-id="30524-144">Code</span></span>  
...
### <a name="code"></a><span data-ttu-id="30524-145">Codice</span><span class="sxs-lookup"><span data-stu-id="30524-145">Code</span></span>  
...
### <a name="code"></a><span data-ttu-id="30524-146">Codice</span><span class="sxs-lookup"><span data-stu-id="30524-146">Code</span></span>  
...
### <a name="code"></a><span data-ttu-id="30524-147">Codice</span><span class="sxs-lookup"><span data-stu-id="30524-147">Code</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="30524-148">Vedere anche</span><span class="sxs-lookup"><span data-stu-id="30524-148">See Also</span></span>  
 [<span data-ttu-id="30524-149">Sicurezza per l'integrazione con CLR</span><span class="sxs-lookup"><span data-stu-id="30524-149">CLR Integration Security</span></span>](../../relational-databases/clr-integration/security/clr-integration-security.md)  
