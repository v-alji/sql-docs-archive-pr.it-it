...
## <a name="spcachehit-event-class-data-columns"></a>Colonne di dati della classe di evento SP:CacheHit  
...
|EventSubClass|`int`|Tipi di sottoclasse di evento.<br /><br /> 1=Accesso al contesto di esecuzione: Piano di esecuzione non compilato trovato nella cache dei piani.<br /><br /> 2 = Accesso piano compilato: Piano compilato trovato nella cache dei piani.|21|Sì|  
...
|ObjectID|`int`|ID assegnato dal sistema alla stored procedure trovata nella cache.|22|Sì|  
|ObjectName|`nvarchar`|Nome dell'oggetto trovato nella cache. Se la colonna ObjectName viene popolata, la colonna TextData non viene popolata.|34|Sì|  
...
|TextData|`ntext`|Testo del codice SQL trovato nella cache. Se la colonna TextData viene popolata, la colonna ObjectName non viene popolata.|1|Sì|  
