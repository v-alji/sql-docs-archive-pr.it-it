...
# <a name="audit-database-scope-gdr-event-class"></a>Audit Database Scope GDR - classe di evento
  La classe di evento **Audit Database Scope GDR** si verifica ogni volta che viene generata un'istruzione GRANT, REVOKE o DENY relativa all'autorizzazione di un'istruzione da parte di un utente in [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] per azioni specifiche di un database, ad esempio la concessione di autorizzazioni in un database.  
...
|**TextData**|**ntext**|Testo SQL dell'istruzione Grant/Revoke/Deny.|1|Sì|  
