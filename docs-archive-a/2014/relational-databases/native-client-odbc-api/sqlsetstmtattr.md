...
|Valore *ValuePtr*|Descrizione|  
|----------------------|-----------------|  
|SQL_DP_ON|Predefinita. Dopo la chiamata della [funzione SQLPrepare](https://go.microsoft.com/fwlink/?LinkId=59360), la preparazione dell'istruzione viene posticipata fino alla chiamata di **SQLExecute** o all'esecuzione dell'operazione di metaproprietà (**SQLDescribeCol** o **SQLDescribeParam**).|  
...
|Valore *ValuePtr*|Descrizione|  
|----------------------|-----------------|  
|SQL_RE_OFF|Predefinita. Il driver non converte i dati di tipo data, ora e valuta in stringhe di caratteri mediante l'impostazione locale del client.|  
...
|SQL_TL_ON|Predefinita. Abilita la registrazione delle operazioni eseguite sui dati di **testo** e di **immagine** .|  
...
|Valore *ValuePtr*|Descrizione|  
|----------------------|-----------------|  
|SQL_HC_OFF|Predefinita. Le colonne FOR BROWSE sono nascoste dal set di risultati.|  
...
|SQL_SS_NAME_SCOPE_EXTENDED|Quando si utilizza la funzionalità colonne di tipo sparse, SQLColumns restituisce tutte le colonne, indipendentemente dall' `column_set` appartenenza.|  
|SQL_SS_NAME_SCOPE_SPARSE_COLUMN_SET|Quando si utilizza la funzionalità colonne di tipo sparse, SQLColumns restituisce solo le colonne che sono membri di sparse `column_set` .|  
