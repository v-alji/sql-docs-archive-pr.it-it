...
#### <a name="using-the-edge-table-format"></a>Utilizzo del formato di tabella edge
 ...
 In una tabella edge viene rappresentata la struttura dettagliata di un documento XML, che include i nomi degli elementi e degli attributi, la gerarchia del documento, gli spazi dei nomi e le istruzioni per l'elaborazione. Il formato di tabella edge consente di ottenere informazioni aggiuntive non esposte tramite le metaproprietà. Per altre informazioni sulle metaproprietà, vedere [Specify Metaproperties in OPENXML](../xml/specify-metaproperties-in-openxml.md).
