...
-   Rende disponibile il mapping tra il prefisso dello spazio dei nomi e l'URI durante le query di [costruzione di codice XML tramite la clausola FOR XML](for-xml-sql-server.md) .  
  
-   Rende disponibile il mapping tra lo spazio dei nomi e l'URI nel contesto dello spazio dei nomi statico dei [metodi con tipo di dati XML](/sql/t-sql/xml/xml-data-type-methods).  
...
```  
 ...
 Di seguito è riportato il risultato. Gli utenti di SQLXML hanno familiarità con questo modello XML. Per altre informazioni, vedere [Concetti relativi alla programmazione SQLXML 4.0](../sqlxml/sqlxml-4-0-programming-concepts.md).  
