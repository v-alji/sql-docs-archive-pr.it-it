...
-   **Soluzioni alternative** : le operazioni di estrazione ed esportazione comporteranno la scrittura di file di dati bcp completamente fedeli nei file con estensione dacpac o BACPAC. Per evitare limitazioni, utilizzare l'utilità della riga di comando BCP.exe di SQL Server per distribuire dati totalmente fedeli in un database di destinazione da un pacchetto di applicazione livello dati.  
