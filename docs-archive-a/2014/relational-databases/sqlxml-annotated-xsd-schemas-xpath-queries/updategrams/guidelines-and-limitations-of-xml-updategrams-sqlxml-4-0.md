...
-   La presenza di caratteri speciali nei dati `ntext` può provocare problemi con SQLXML 4.0 a causa delle limitazioni applicate al confronto per i tipi BLOB. Ad esempio, l'uso di "[Serializable]" nel **\<before>** blocco di un updategram quando viene usato nel controllo della concorrenza di una colonna di `ntext` tipo avrà esito negativo con la descrizione dell'errore SQLOLEDB seguente:  
