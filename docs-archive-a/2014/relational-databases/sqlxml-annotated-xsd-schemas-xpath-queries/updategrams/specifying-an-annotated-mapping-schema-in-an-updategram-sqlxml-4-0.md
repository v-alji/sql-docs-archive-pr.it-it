...
## <a name="dealing-with-data-types"></a>Gestione dei tipi di dati  
 Se lo schema specifica il `image` `binary` `varbinary` [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] tipo di dati, o (utilizzando `sql:datatype` ) e non specifica un tipo di dati XML, l'updategram presuppone che il tipo di dati XML sia `binary base 64` . Se i dati sono di tipo `bin.base`, è necessario specificare in modo esplicito il tipo (`dt:type=bin.base` o `type="xsd:hexBinary"`).  
